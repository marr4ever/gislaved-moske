...
import { convertToDailyPrayers, getMonthlyPrayerTimes, hasDataForDate } from "@/lib/accurate-prayer-times"
...
export async function fetchPrayerTimes(date: Date): Promise<DailyPrayers> {
  ...
  if (hasDataForDate(date)) {
    const accurateTimes = convertToDailyPrayers(date)
    if (accurateTimes) {
...
export async function fetchMonthlyPrayerTimes(year: number, month: number): Promise<DailyPrayers[]> {
  ...
  const accurateTimes = getMonthlyPrayerTimes(year, month)
